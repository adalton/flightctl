...
const (
	...
	// DefaultCertRenewalThresholdDays is the default number of days before expiration to trigger renewal
	DefaultCertRenewalThresholdDays = 30
	// DefaultCertRetryInitialInterval is the default initial retry interval for certificate renewal
	DefaultCertRetryInitialInterval = util.Duration(1 * time.Minute)
	// DefaultCertRetryMaxInterval is the default maximum retry interval for certificate renewal
	DefaultCertRetryMaxInterval = util.Duration(24 * time.Hour)
	// DefaultCertRetryBackoffMultiplier is the default exponential backoff multiplier
	DefaultCertRetryBackoffMultiplier = 2.0
	// DefaultCertMonitorIntervalSeconds is the default interval for checking certificate expiration
	DefaultCertMonitorIntervalSeconds = 60
...
type Config struct {
	...
	// CertRotation holds all certificate rotation configuration
	CertRotation CertRotationConfig `json:"cert-rotation,omitempty"`
...
// CertRotationConfig holds configuration for automatic certificate rotation
type CertRotationConfig struct {
	// Enabled indicates whether automatic certificate rotation is enabled
	Enabled bool `json:"enabled,omitempty"`
	// RenewalThresholdDays is the number of days before expiration to trigger renewal
	RenewalThresholdDays int `json:"renewal-threshold-days,omitempty"`
	// RetryInitialInterval is the initial retry interval for failed renewal attempts
	RetryInitialInterval util.Duration `json:"retry-initial-interval,omitempty"`
	// RetryMaxInterval is the maximum retry interval for failed renewal attempts
	RetryMaxInterval util.Duration `json:"retry-max-interval,omitempty"`
	// RetryBackoffMultiplier is the exponential backoff multiplier for retries
	RetryBackoffMultiplier float64 `json:"retry-backoff-multiplier,omitempty"`
	// MonitorIntervalSeconds is the interval for checking certificate expiration
	MonitorIntervalSeconds int `json:"monitor-interval-seconds,omitempty"`
}
...
func NewDefault() *Config {
	c := &Config{
		...
		CertRotation: CertRotationConfig{
			Enabled:                false,
			RenewalThresholdDays:   DefaultCertRenewalThresholdDays,
			RetryInitialInterval:   DefaultCertRetryInitialInterval,
			RetryMaxInterval:       DefaultCertRetryMaxInterval,
			RetryBackoffMultiplier: DefaultCertRetryBackoffMultiplier,
			MonitorIntervalSeconds: DefaultCertMonitorIntervalSeconds,
		},
