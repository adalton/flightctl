...
// Defines values for CertificateRenewalRequestSecurityProofType.
const (
	BootstrapCert  CertificateRenewalRequestSecurityProofType = "bootstrap_cert"
	TpmAttestation CertificateRenewalRequestSecurityProofType = "tpm_attestation"
	ValidCert      CertificateRenewalRequestSecurityProofType = "valid_cert"
)

// Defines values for CertificateRenewalResponseStatus.
const (
	Completed CertificateRenewalResponseStatus = "completed"
	Failed    CertificateRenewalResponseStatus = "failed"
)
...
// CertificateRenewalRequest Request to renew a device's management certificate. The device authenticates using one of three methods depending on its current certificate state.
type CertificateRenewalRequest struct {
	// Csr PEM-encoded Certificate Signing Request for the new certificate.
	Csr string `json:"csr"`

	// OldCertificateSerial Serial number of the certificate being renewed (optional, for audit purposes).
	OldCertificateSerial *string `json:"oldCertificateSerial,omitempty"`

	// RequestId Unique identifier for this renewal request generated by the device.
	RequestId openapi_types.UUID `json:"requestId"`

	// SecurityProofType Authentication method used for this renewal request. 'valid_cert' uses the current management certificate, 'bootstrap_cert' uses the bootstrap certificate for expired cert recovery, 'tpm_attestation' uses TPM attestation for devices without valid certificates.
	SecurityProofType CertificateRenewalRequestSecurityProofType `json:"securityProofType"`

	// TpmAttestationProof TPM attestation proof for expired certificate recovery. Used when the device has an expired management certificate but valid TPM identity.
	TpmAttestationProof *TPMAttestationProof `json:"tpmAttestationProof,omitempty"`
}

// CertificateRenewalRequestSecurityProofType Authentication method used for this renewal request. 'valid_cert' uses the current management certificate, 'bootstrap_cert' uses the bootstrap certificate for expired cert recovery, 'tpm_attestation' uses TPM attestation for devices without valid certificates.
type CertificateRenewalRequestSecurityProofType string

// CertificateRenewalResponse Response to a certificate renewal request containing the new certificate.
type CertificateRenewalResponse struct {
	// Certificate PEM-encoded renewed certificate (only present if status is 'completed').
	Certificate string `json:"certificate"`

	// IssuedAt Timestamp when the certificate was issued.
	IssuedAt time.Time `json:"issuedAt"`

	// NotAfter Certificate validity end time (expiration).
	NotAfter time.Time `json:"notAfter"`

	// NotBefore Certificate validity start time.
	NotBefore time.Time `json:"notBefore"`

	// RequestId The request ID from the renewal request, for correlation.
	RequestId openapi_types.UUID `json:"requestId"`

	// SerialNumber Serial number of the newly issued certificate.
	SerialNumber string `json:"serialNumber"`

	// Status Status of the renewal request.
	Status CertificateRenewalResponseStatus `json:"status"`
}

// CertificateRenewalResponseStatus Status of the renewal request.
type CertificateRenewalResponseStatus string
...
// TPMAttestationProof TPM attestation proof for expired certificate recovery. Used when the device has an expired management certificate but valid TPM identity.
type TPMAttestationProof struct {
	// AttestationData Base64-encoded TPM attestation data proving device identity.
	AttestationData string `json:"attestationData"`

	// Signature Base64-encoded signature of the attestation data.
...
// RenewDeviceCertificateJSONRequestBody defines body for RenewDeviceCertificate for application/json ContentType.
type RenewDeviceCertificateJSONRequestBody = CertificateRenewalRequest
